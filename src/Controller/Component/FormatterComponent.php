...
class FormatterComponent extends Component {
    ...
    public function extensionFromMimeType($selected_mime_type) {
        $mime_types = array(
        ...
        foreach ($mime_types as $key => $type){
            if(strpos($type, $selected_mime_type) !== false)
                return $key;
        }
    ...
    public function getExtensionFromFileName($filename) {
        ...
        return isset($fileName[count($fileName)-1]) ? '.'.$fileName[count($fileName)-1] : '';
    }
}
